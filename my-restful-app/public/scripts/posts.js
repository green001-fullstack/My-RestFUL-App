...
document.addEventListener('click', async (e) => {
    ...
    if (e.target.classList.contains('edit-post')) {
        ...
        contentDiv.replaceChild(textarea, paragraph);

        e.target.textContent = 'Save';
        e.target.classList.replace('edit-post', 'save-post');

    } else if (e.target.classList.contains('save-post')) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                newParagraph.textContent = updatedPost.body;

                const contentDiv = postElement.querySelector('.post-content');
                contentDiv.replaceChild(newParagraph, textarea);

                e.target.textContent = 'Edit';
                e.target.classList.replace('save-post', 'edit-post');
            } else {
                const error = await response.json();
                alert(error.error);
            }
